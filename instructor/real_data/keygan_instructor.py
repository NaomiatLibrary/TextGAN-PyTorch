...
class KeyGANInstructor(BasicInstructor):
    ...
    def variation(self, g_step, criterionG):
        ...
        for step in range(g_step):
            ...
            gen_samples = self.gen.sample_from_keyword(keywords,cfg.batch_size, cfg.batch_size, one_hot=True)
            if cfg.CUDA:
                # real_samples, gen_samples = real_samples.cuda(), gen_samples.cuda()
    ...
    def prepare_eval_fake_data(self):
        with torch.no_grad():
            keywords = [random.randrange(cfg.vocab_size)]#これでいいのか？
            self.eval_fake_samples = self.gen.sample_from_keyword(keywords,cfg.eval_b_num * cfg.batch_size,
    ...
    def _save(self, phase, epoch):
        ...
        keywords = [random.randrange(cfg.vocab_size)]#これでいいのか？
        samples = self.gen.sample_from_keyword(keywords,cfg.batch_size, cfg.batch_size)
